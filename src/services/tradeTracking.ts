 ...
 * Fetch whitelist
 */
export async function fetchWhitelist(includeInactive: boolean = false) {
  try {
    const response = await fetch(`${API_BASE}/whitelist?includeInactive=${includeInactive}`, {
      credentials: 'include'
    })
    ...
    return { success: false, error: error.message, whitelist: [], total: 0 }
  }
}

/**
 * Get specific whitelist token
 */
export async function getWhitelistToken(tokenAddress: string) {
  try {
    const response = await fetch(`${API_BASE}/whitelist/${encodeURIComponent(tokenAddress)}`, {
      credentials: 'include'
    })

    if (!response.ok) {
      const error = await response.json()
      throw new Error(error.error || 'Token not found in whitelist')
    }
  ...
  try {
    const response = await fetch(`${API_BASE}/whitelist/add`, {
      ...
      body: JSON.stringify({
        ...
        start_track_datetime: startTrackDatetime,
        period,
        unit
      })
    })
 ...
 */
export async function removeFromWhitelist(tokenAddress: string) {
  try {
 ...
 */
export async function checkWhitelistStatus(tokenAddress: string) {
  try {
    const response = await fetch(`${API_BASE}/whitelist/check`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
    ...
    if (!response.ok) {
      const error = await response.json()
      throw new Error(error.error || 'Failed to check whitelist status')
    }
    ...
    return { success: false, error: error.message, isWhitelisted: false }
  }
}
 ...
 */
export async function cleanupExpiredWhitelist() {
  try {
    const response = await fetch(`${API_BASE}/whitelist/cleanup`, {
      method: 'POST',
      credentials: 'include'
    })
    ...
    if (!response.ok) {
      const error = await response.json()
      throw new Error(error.error || 'Failed to cleanup expired whitelist')
    }
